...
async function loadAllData() {
    try {
        ...
        todoList = data.todoListDB || []; // 新增：加载待办清单
                // 恢复壁纸和头像
        ...
        quizHistory = data.quizHistoryDB || [];
        // 强制刷新问卷显示
        if (data.quizHistoryDB) {
            quizHistory = data.quizHistoryDB;
        }
        ...
        if (data.quizHistoryDB) { quizHistory = data.quizHistoryDB; }
        // 强制刷新问卷数据
        if (data.quizHistoryDB) {
            quizHistory = data.quizHistoryDB;
        }
...
function syncToLocalStorage(data) {
    const keys = ['chatHistoryDB', 'replyMode', 'replyLibraryDB', 'myStickers', 'youStickers',
        ...
        'rhythmSettings', 'tickleSettings', 'letterHistoryDB', 'todoListDB' // 新增
    ];
    keys.forEach(key => {
...
function loadFromLocalStorage() {
    ...
    todoList = JSON.parse(localStorage.getItem('todoListDB')) || []; // 新增
}
...
function renderTodoList() {
    ...
    const uncompleted = todoList.filter(item => !item.completed);
    const completed = todoList.filter(item => item.completed);
    
    // 把未完成的排在前面，已完成的排在后面
    const sortedList = [...uncompleted, ...completed];

    if (sortedList.length === 0) {
        container.innerHTML = '<div style="text-align:center; color:#888; font-size:14px; padding: 20px 0;">还没有待办事项哦，快添加一个吧~</div>';
        return;
    }
    ...
    sortedList.forEach(item => {
        ...
        itemDiv.onclick = () => toggleTodoItem(item.id);
...
function addTodoItem() {
    ...
    todoList.unshift(newItem); // 添加到最前面
    saveToBackend('todoListDB', todoList);
    
    // 自动在回复库里增加提醒
    const reminderText = `不要忘记${text}`;
    const reminderCategory = "待办提醒";
    if (!replyLibrary[reminderCategory]) {
        replyLibrary[reminderCategory] = "";
    }
    // 确保前面有换行符
    if (replyLibrary[reminderCategory].length > 0 && !replyLibrary[reminderCategory].endsWith('\n')) {
        replyLibrary[reminderCategory] += '\n';
    }
    replyLibrary[reminderCategory] += reminderText;
    saveToBackend('replyLibraryDB', replyLibrary);
    renderCategorySelect(); // 刷新分类下拉框，万一新建了分类呢
...
function toggleTodoItem(id) {
    ...
    // 根据完成状态，自动增删回复库里的提醒
    const reminderText = `不要忘记${item.text}`;
    const reminderCategory = "待办提醒";

    if (item.completed) { // 如果是完成待办
        if (replyLibrary[reminderCategory]) {
            let phrases = replyLibrary[reminderCategory].split('\n');
            phrases = phrases.filter(p => p.trim() !== reminderText.trim());
            replyLibrary[reminderCategory] = phrases.join('\n');
            // 如果这个分类空了，就把它删掉
            if(replyLibrary[reminderCategory].trim() === '') {
                delete replyLibrary[reminderCategory];
            }
            saveToBackend('replyLibraryDB', replyLibrary);
            renderCategorySelect();
        }
        ...
        // 避免重复添加
        if (!replyLibrary[reminderCategory].includes(reminderText)) {
            replyLibrary[reminderCategory] += reminderText;
            saveToBackend('replyLibraryDB', replyLibrary);
            renderCategorySelect();
        }
...
function stopAudioPlayback() {
    ...
    currentlyPlayingAudio = { id: null, playerElement: null };
}
...
async function triggerBotReply() {
    ...
    if (Math.random() < 0.3 && chatHistory.length > 0) {
        ...
        if (myMsgs.length > 0) {
            ...
            else if (randomMsg.type === 'voice') randomQuoteText += `[语音] ${randomMsg.content.text}`;
...
function addNewCategory() {
    ...
    if (newCat && newCat.trim() !== "" && newCat.trim() !== "待办提醒") {
        if (!replyLibrary[newCat.trim()]) { replyLibrary[newCat.trim()] = "";
            currentCategory = newCat.trim();
            renderCategorySelect(); } else { alert("分类已存在"); }
    } else if (newCat.trim() === "待办提醒") {
        alert("这个是系统保留分类名哦宝宝，换一个吧~");
    }
}

function saveCategory() {
    if (currentCategory === "待办提醒") {
        alert("这个是系统自动管理的分类哦，不能手动修改~");
        replyTextarea.value = replyLibrary[currentCategory] || ""; // 恢复原状
        return;
    }
