...
window.onload = () => {
    ...
    renderStatusCategorySelect(); // 新增：渲染状态分类
    checkAndUpdateStatus(); // 新增：检查并更新状态
    setInterval(checkAndUpdateStatus, 60000); // 新增：每分钟检查一次是否需要刷新状态
    renderStickers();
    renderChatHistory();
...
function toggleStatusPopup(event) {
    event.stopPropagation(); // 阻止冒泡，防止触发 body 的点击事件
...
function sendLetter() {
    ...
    letterHistory.unshift(newLetter); // 最新的信件放在最前面
...
function triggerBotReply() {
    ...
    if (Math.random() < 0.03) {
        ...
        const text = `${name} ${action} 你${suffix}`;
        messageQueue.push({ type: 'tickle', content: text });
    }

    if (currentMode === 'combined') {
        let finalReplyText = allPhrases.length > 0 ? Array.from({length: replyCount}, () => allPhrases[Math.floor(Math.random() * allPhrases.length)].trim()).join(' ') : '词库为空';
        if (youStickers.length > 0 && Math.random() < 0.3) {
            if(rhythmSettings.mixEmoji) {
                messageQueue.push({ type: 'mixed', content: { text: finalReplyText, img: youStickers[Math.floor(Math.random() * youStickers.length)] } });
            } else {
                messageQueue.push({ type: 'img', content: youStickers[Math.floor(Math.random() * youStickers.length)] });
                messageQueue.push({ type: 'text', content: finalReplyText });
            }
        } else { messageQueue.push({ type: 'text', content: finalReplyText }); }
    } else {
        for (let i = 0; i < replyCount; i++) {
            if (youStickers.length > 0 && Math.random() < 0.2) {
                let phrase = allPhrases.length > 0 ? allPhrases[Math.floor(Math.random() * allPhrases.length)].trim() : '词库为空';
                if(rhythmSettings.mixEmoji) messageQueue.push({ type: 'mixed', content: { text: phrase, img: youStickers[Math.floor(Math.random() * youStickers.length)] } });
                else messageQueue.push({ type: 'img', content: youStickers[Math.floor(Math.random() * youStickers.length)] });
            } else {
                messageQueue.push({ type: 'text', content: allPhrases.length > 0 ? allPhrases[Math.floor(Math.random() * allPhrases.length)].trim() : '词库为空' });
            }
        }
    }
    ...
    let textMsgIndex = messageQueue.findIndex(m => m.type !== 'tickle');
    if (textMsgIndex !== -1 && randomQuoteText) {
        messageQueue[textMsgIndex].quote = randomQuoteText;
    }
...
function processMessageQueue(queue) {
    ...
    setTimeout(() => {
        ...
        if(queue.length > 0) setTimeout(() => processMessageQueue(queue), 400); 
